...
To run the testbench it's sufficient to only read the first 6 words. The SPI in the testbench is preloaded with the following 6 assembler instructions of the program `leds.asm` translated into `HACK` machine language:
...
If  `leds.asm` is working, you are ready to start implementing the operating system Jack OS. Proceed to project `07_Operating_System`  and come back later to implement the last to I/O devices `LCD` and `RTP`  to connect the screen with resitive touch panel `MOD-LCD2.8RTP`.